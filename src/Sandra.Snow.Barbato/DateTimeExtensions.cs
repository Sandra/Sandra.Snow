namespace Sandra.Snow.Barbato
{
    ...
    public static class DateTimeExtensions
    {
        public static string ToRelativeTime(this DateTime pastDateTime)
        {
            ...
            if (delta < 1 * MINUTE)
            {
                return ts.Seconds == 1 ? "one second ago" : "about " + ts.Seconds + " seconds ago";
            }
            if (delta < 2 * MINUTE)
            {
            ...
            if (delta < 12 * MONTH)
            {
                ...
                return months <= 1 ? "one month ago" : "about " + months + " months ago";
            }
            else
            {
                int years = Convert.ToInt32(Math.Floor((double)ts.Days / 365));
                return years <= 1 ? "one year ago" : "about " + years + " years ago";
            }
        }
    }
}